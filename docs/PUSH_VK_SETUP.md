...
VK_COMMUNITY_TOKEN=...
VK_GROUP_ID=...
PUBLIC_URL=https://...backend...     # только фото/QR, НЕ ссылка в push
VK_MINI_APP_URL=https://vk.ru/app54662212   # ссылка в ЛС сообщества (fallback)
```

Тексты transactional-пушей (задания, медали, точки) — в `mashuk-backend/src/services/pushCopy.ts`.  
В конце ЛС добавляется блок:

```
Открыть приложение:
https://vk.ru/app54662212
```
