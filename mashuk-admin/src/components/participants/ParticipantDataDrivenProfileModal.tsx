...
export function ParticipantDataDrivenProfileModal({
  participantId,
  participantName,
  open,
  onClose,
  ...
  useEffect(() => {
    ...
    setError(null);
    setHtml(null);
    adminFetchHtml(`/participants/${participantId}/profile-3`)
      .then((doc) => {
        if (cancelled) return;
        setHtml(doc);
      })
  ...
  const title = participantName
    ? `Профиль участника 3 · ${participantName}`
    : 'Профиль участника 3';
    ...
    >
      ...
      >
        <div className="adm-final-profile-topbar">
          <div>
            ...
            <div className="adm-muted" style={{ fontSize: 12, marginTop: 2 }}>
              Data-driven HTML-профиль · поведение, психографика, рекомендации
            </div>
          </div>
        ...
        <div className="adm-final-profile-body">
          {loading && <div className="adm-muted" style={{ padding: 24 }}>Собираем data-driven профиль…</div>}
          {error && (
