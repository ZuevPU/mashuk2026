...
export function ParticipantAnalyticalProfileModal({
  participantId,
  participantName,
  open,
  onClose,
  ...
  useEffect(() => {
    ...
    setError(null);
    setHtml(null);
    adminFetchHtml(`/participants/${participantId}/profile-2`)
      .then((doc) => {
        if (cancelled) return;
        setHtml(doc);
      })
  ...
  const title = participantName
    ? `Профиль участника 2 · ${participantName}`
    : 'Профиль участника 2';
    ...
    >
      ...
      >
        <div className="adm-final-profile-topbar">
          <div>
            ...
            <div className="adm-muted" style={{ fontSize: 12, marginTop: 2 }}>
              Аналитический HTML-профиль · путь, смыслы, сильные стороны
            </div>
          </div>
        ...
        <div className="adm-final-profile-body">
          {loading && <div className="adm-muted" style={{ padding: 24 }}>Собираем аналитический профиль…</div>}
          {error && (
