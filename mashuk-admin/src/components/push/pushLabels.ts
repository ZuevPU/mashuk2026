...
/** Личные сервисные пуши — не рассылка. Массовые слоты выключены. */
export const PUSH_SYSTEM_EVENTS = [
  {
    group: 'Точки дня и программа',
    items: [
      'Массовые письма по вопросам, вечерней анкете и программе — только руками из «По дням».',
      'Публикация вопроса или задания сама ничего не шлёт.',
    ],
  },
  {
    group: 'Задания (личное, не рассылка)',
    items: [
      'Отправка на проверку, одобрение, отклонение — участнику.',
...
export function deliveryStatusShort(status: string | null | undefined): string {
  ...
  if (status.includes(';')) {
    if (/sent_mini/.test(status) && /sent_community/.test(status)) return '✓ Mini + ЛС сообщества';
    if (/sent_mini/.test(status) && /error:|skipped_no_community/.test(status)) {
