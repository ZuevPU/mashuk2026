...
const RULES = [
  ...
  { match: /Хранится: participants\.qr_url/, status: 'Да', noteAppend: 'qrToken + buildParticipantQrUrl' },
  { match: /Формат URL: deep-link для приложения волонтёра/, status: 'Да', noteAppend: 'Volunteer panel + qrDeepLink' },
  { match: /Сценарий: волонтёр сканирует QR участника/, status: 'Да', noteAppend: 'volunteerController + ParticipantCard QR' },
  { match: /В личном профиле участника — вкладка «Мой QR»/, status: 'Да', noteAppend: 'Profile settings QR block' },
  { match: /Регенерация QR при подозрении/, status: 'Да', noteAppend: 'POST /profile/regenerate-qr' },

  // —— 14+ дубли ——
  { match: /Раздел «Система баллов» в админ-панели/, status: 'Да', noteAppend: 'LevelsTab stakes + bonus rules' },
  { match: /Для каждого действия задаётся: название/, status: 'Да', noteAppend: 'action-catalog PATCH' },
  { match: /Действия для счётчика «Путь»/, status: 'Да', noteAppend: 'pointsLine path' },
  { match: /Действия для счётчика «Опыт»/, status: 'Да', noteAppend: 'pointsLine experience' },
  { match: /Действия для счётчика «Идей»/, status: 'Да', noteAppend: 'piggybank tag idea' },
  { match: /Система автоматически учитывает ограничения/, status: 'Да', noteAppend: 'pointsAwarded caps' },
  { match: /Все настройки без участия разработчика/, status: 'Да', noteAppend: 'LevelsTab + forum_settings' },
  ...
  { match: /Поле «Условие получения»/, status: 'Да', noteAppend: 'ruleMetric + manual award in ParticipantCard' },
  { match: /Поле «Иконка» — изображение/, status: 'Да' },
  { match: /Поле «Категория» — привязка/, status: 'Да', noteAppend: 'medal category field' },
  ...
  { match: /Возможность настройки порогов уровней «Пути»/, status: 'Да', noteAppend: 'ThresholdEditor path_level' },
  { match: /Возможность настройки порогов уровней «Опыта»/, status: 'Да', noteAppend: 'ThresholdEditor exp_level' },
  { match: /Настройка типов рейтинговых таблиц/, status: 'Да', noteAppend: 'leaderboardScopes в LevelsTab' },
