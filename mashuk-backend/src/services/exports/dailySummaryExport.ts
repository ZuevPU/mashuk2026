...
export async function writeDailySummaryExport(res: Response, filters: Filters): Promise<void> {
  ...
  if (filters.activityQ) {
    const q = filters.activityQ.toLowerCase();
    filtered = filtered.filter(r =>
      (r.p?.workplace ?? '').toLowerCase().includes(q)
      || (r.p?.position ?? '').toLowerCase().includes(q),
    );
  }

  const wb = await createWorkbook();
  addReadmeSheet(wb, ['Итоги дня: одна строка = участник × день (сданная итоговая анкета).']);
