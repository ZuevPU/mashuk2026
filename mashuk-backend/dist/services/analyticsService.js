...
export async function recalculateDailyStats(_direction = 'all') {
    ...
    const directions = ['all', ...new Set(allParticipants.map(p => p.direction).filter(Boolean))];
    for (const dir of directions) {
        ...
        const dirCheckins = dirAnswers.filter(a => {
            const data = a.answerData;
            return data && typeof data.energy === 'number';
        });
        const dirAvgEnergy = dirCheckins.length
            ? Math.round(dirCheckins.reduce((sum, a) => sum + (a.answerData.energy || 0), 0) / dirCheckins.length)
            : 0;
        const dirEmotions = {};
        for (const a of dirCheckins) {
            const emo = a.answerData.emotion;
            if (emo)
                dirEmotions[emo] = (dirEmotions[emo] || 0) + 1;
        }
