...
describe('E2E participant + admin flow', { skip: !process.env.DATABASE_URL }, () => {
    ...
    it('cleanup and register E2E participant', async () => {
        ...
        const existing = list.body.participants?.find((p) => p.vkId === E2E_VK_ID);
        if (existing) {
            await request(app)
    ...
    it('participant creates activity', async () => {
        ...
        const q = questions.body.questions?.find((x) => x.status === 'available');
        if (q) {
        ...
        const task = tasks.body.tasks?.find((t) => t.status === 'available');
        if (task) {
    ...
    it('admin sees data, exports CSV, recalculates analytics', async () => {
        ...
        assert.ok(participants.body.participants.some((p) => p.vkId === E2E_VK_ID));
        ...
        assert.ok(exchange.body.questions?.some((q) => q.text?.includes('E2E exchange')));
        for (const path of [
    ...
    it('admin push send writes to push_log', async () => {
        ...
        const p = list.body.participants.find((x) => x.vkId === E2E_VK_ID);
        assert.ok(p);
        ...
        assert.ok(log.body.log?.some((l) => l.text === 'E2E push test'));
    });
});
