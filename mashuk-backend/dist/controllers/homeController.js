...
import { dayFocus, questions, answers, tasks, taskSubmissions, piggybank, events, } from '../db/schema.js';
    ...
    try {
        ...
        const missed = publishedQuestions.filter(q => {
            if (answeredIds.has(q.id))
                return false;
            if (q.closeTime && q.closeTime < now)
                return false;
        ...
        const availableTasks = await db.select().from(tasks)
            .where(and(or(isNull(tasks.dayNumber), eq(tasks.dayNumber, settings.currentDay ?? 1)), or(isNull(tasks.publishTime), lte(tasks.publishTime, now))));
        ...
        const schedule = [];
        const nowEvent = dayEvents.find(e => e.startTime && e.startTime <= now && (!e.endTime || e.endTime >= now));
        if (nowEvent) {
            schedule.push({
                kind: 'now',
...
export const quickPiggybank = async (req, res) => {
    try {
