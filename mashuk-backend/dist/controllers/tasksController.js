...
function getTaskStatus(task, submission, now) {
...
export const listTasks = async (req, res) => {
    try {
        ...
        const allTasks = await db.select().from(tasks)
            .where(and(or(isNull(tasks.dayNumber), eq(tasks.dayNumber, settings.currentDay ?? 1))));
        ...
        let result = allTasks.map(t => {
            ...
            if (t.hideUntilPublish && t.publishTime && t.publishTime > now)
                return null;
        ...
        if (filter === 'active')
            result = result.filter(t => t.status === 'available' || t.canResubmit);
        if (filter === 'done')
...
export const submitTask = async (req, res) => {
    try {
        ...
        const [existing] = await db.select().from(taskSubmissions)
            .where(and(eq(taskSubmissions.participantId, req.participant.id), eq(taskSubmissions.taskId, taskId))).limit(1);
