...
import { questions, questionOptions, answers, exchangeQuestions, exchangeAnswers, participants, } from '../db/schema.js';
import { countWords } from '../services/helpers.js';
import { awardPoints } from '../services/pointsService.js';
export const listForumQuestions = async (req, res) => {
    try {
        const now = new Date();
        ...
        const result = list.map(q => {
            ...
            if (q.publishTime && q.publishTime > now)
                status = 'soon';
            else if (q.closeTime && q.closeTime < now)
                status = 'closed';
...
export const submitAnswer = async (req, res) => {
    try {
        ...
        const [existingAnswer] = await db.select().from(answers)
            .where(and(eq(answers.participantId, req.participant.id), eq(answers.questionId, questionId))).limit(1);
...
export const listExchange = async (req, res) => {
    try {
        const list = await db.select({
            ...
            .where(or(eq(exchangeQuestions.moderationStatus, 'approved'), eq(exchangeQuestions.participantId, req.participant.id)));
...
export const createExchangeQuestion = async (req, res) => {
    try {
...
export const answerExchange = async (req, res) => {
    try {
        const questionId = Number(req.params.id);
        const { text } = req.body;
...
export const reactExchangeAnswer = async (req, res) => {
    try {
        ...
        const reactions = existing.reactions || { likes: 0, discuss: 0 };
        if (type === 'like')
            reactions.likes = (reactions.likes || 0) + 1;
        else
            reactions.discuss = (reactions.discuss || 0) + 1;
